...
# data can be altered.
Base.setindex!(s::ExtensionSequence, val, k::Int) = setindex!(data(s), val, mapindex(s, k))
...
Base.similar(s::CompactSequence, v::AbstractVector, optional...) =
...
Base.reverse(s::CompactSequence) = CompactSequence(flipdim(data(s), 1), -rightindex(s))

# Take element-wise conjugates
Base.conj(s::CompactSequence) = CompactSequence(conj(data(s)), leftindex(s))

Base.ctranspose(s::CompactSequence) = CompactSequence(conj(flipdim(data(s),1)), -rightindex(s))
...
Base.similar(s::PeriodicExtension, v::AbstractVector, optional...) =
...
Base.getindex(s::PeriodicExtension, k::Int) = getindex(data(s), mapindex(s, k))
...
Base.similar(s::SymmetricExtension{T,PL,PR,SL,SR}, v::AbstractVector{S}, optional...) where {S,T,PL,PR,SL,SR} =
